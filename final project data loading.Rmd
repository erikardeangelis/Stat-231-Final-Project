...
server <- function(input, output, session) {
    output$result <- renderPrint({
      result <- paste("The", statefooddata(input$e0))
      result
      }
)
}
...
saveGIF({
  ...
  for (i in unique(outbreaks_map$bi_year)) {
    ...
    p <- ggplot() + geom_polygon(data=outbreaks_map, aes(x=long, y=lat, group = group, fill=VALUE, colour="white")) + xlab("Longitude") + ylab("Latitude") + theme(plot.title = element_text(hjust = 0.5, face = "bold")) + scale_fill_gradient(low = "white", high = "blue", na.value = "black") + ggtitle(i)
...
#gganimate_save(g, filename = "maps.mp4", saver = "mp4")
