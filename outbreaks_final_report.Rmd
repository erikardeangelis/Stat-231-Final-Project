...
This animated map depicts the number of outbreaks that have occurred for every two-year period between 2000 and 2015 in each U.S. state, with the number of outbreaks represented by the color of each state (deeper red indicating more outbreaks). Certain states in the dataset were missing the number of outbreaks data for particular years, those those states with missing data are not filled in on the map. This animation was created with the gganimate package, which creates an .mp4 file as an output (which is located in the github repository). 
...
We also wanted to create a visual that would represent the total number of outbreaks for each U.S. state for all of the years in 2000-2015. Similarly to the animated map, this map represents the number of outbreaks by color, with a deeper red indicated more outbreaks in that particular state. 
...
We hypothesized that there would be a positive linear association between number of restaurant violations and number of outbreaks. We considered that more restaurant violations might mean more cases one, comparing the number of foodborne outbreaks to the number of restaurant violations per month. 
...
To run the Shiny application, there is a separate .Rmd file included in the github repository called shiny_app_code.Rmd that can be accessed here: 
